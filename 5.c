...
#define SET_SEG_DIRECTION             0xA0 // нормальный пор€док записи столбцов
#define SET_COM_DIRECTION             0xC0 // нормальный пор€док отображени€ строк
...
#define SET_INVERSE_DISPLAY           0xA6 // »нверсный режим. 0 Ч нормальное отображение содержимого пам€ти, 1 Ч инверсное.
...
int main(void) {
    ...
    UCB1CTL0 = UCCKPH | UCMSB | UCMST | UCMODE_0 | UCSYNC; // фаза ти, пор€док передачи - MSB, Master, 3-pin, синхронный режим
    UCB1CTL1 &= ~UCSWRST; // разрешение работы модул€ USCI
    UCB1IFG &= ~UCRXIFG; //флаг прерывани€ приЄмника
...
void CMA3000_init(void) {
    ...
    P3DIR  |= (BIT3 | BIT6);    // режим SIMO и напр€жение питани€ PWM
    P3DIR  &= ~BIT4;        // SOMI
